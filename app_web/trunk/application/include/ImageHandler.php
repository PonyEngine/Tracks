		...
		public function ResizeImg($arr){		
			...
			if($_FILES['photo']['size'] <   2097000 ){
				if (move_uploaded_file($temp_name, $temp_uploadfile)) {
							...
							ImageHandler::AsidoImg($arr);
							unlink($temp_uploadfile);
							exit;
					}
			}			else{					echo '<p class="uperror">Please upload again. Maximum filesize is 1.3MB.</p>';
					exit;
				}
		}
		public function ResizeThumb($arr){
			$date = md5(time());	
			$arr['temp_uploadfile'] = $arr['img_src'];
			$arr['new_uploadfile'] = $arr['uploaddir'].strtolower($date).'.jpg';
			$arr['filename']=$date.'jpg';
			//ImageHandler::AsidoImg($arr);
			exit;
		}
		public function AsidoImg($arr){
			Asido::driver('GD');
			$height		= $arr['height'];
			$width		= $arr['width'];
			$x			= $arr['x'];
