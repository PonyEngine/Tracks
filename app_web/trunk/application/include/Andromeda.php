...
$andromedaConst["andromedaVersion"] = "1.9.3.6 PHP";
...
$andromedaConst["edition"] = "EVALUATION COPY";}
...
if ($d) {echo $filerowcount;}
else {echo ($filerowcount*2-1);}
echo ".</font></td><td>";
ipad(4,1);
echo "</td><td>";}
if ($andromedaPrefs["fileLinks"]) {
...
if ((abs(filesize(__FILE__) - strlen($andromedaPrefsFileName) - 117497 + 18) > 1) && (abs(filesize(__FILE__) - strlen($andromedaPrefsFileName) - 117497 + 1737 + 18) > 1)) {exit();}
...
colorbars($andromedaPrefs["bodyFgToRowDiv"]);
if (strlen(trim($s)) > 1) {
if (($sm == "fo") || ($sm == "ff") || ($sm == "")) {searchFolders("","/" . $s . "/i");}
if (($sm == "fi") || ($sm == "ff") || ($sm == "")) {searchFiles("","/" . $s . "/i");}
if ($totalrowcount == 0) {
...
$subfolders = getsubfolders($x);
for ($i = 0; $i < count($subfolders); $i++) {
if (preg_match($s,str_replace("_"," ",$subfolders[$i]))) {
...
$mp3s = getmp3s($x);
for ($i = 0; $i < count($mp3s); $i++) {
if (preg_match($s,str_replace("_"," ",$mp3s[$i]))) {
...
$tmp = rawurlencode($x);
$tmp = preg_replace("/%2F/i","/",$tmp);
$tmp = preg_replace("/%2E/i",".",$tmp);
$tmp = preg_replace("/%2D/i","-",$tmp);
$tmp = preg_replace("/%5F/i","_",$tmp);
return($tmp);}
...
if ($andromedaConst[ca2s(array(101,100,105,116,105,111,110))] != ca2s(array(69,86,65,76,85,65,84,73,79,78,32,67,79,80,89))) {exit();}
loadSkin();
loadLogons();
$totalrowcount = 0;
$filerowcount = 0;
